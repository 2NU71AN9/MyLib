...
class SLCoreDataManager {
    ...
    /// - Parameter model: 表名
    /// - Returns: 对象
    func table2Model<T>(model: T.Type) -> T? {
    ...
    ///   - success: 成功
    ///   - failure: 失败
    func save<T>(model: T.Type, content: (T?) -> Void, success: (() -> Void)?, failure: ((Error) -> Void)?) {
    ...
    ///   - failure: 失败
    func fetch<T>(table: T.Type, fetchRequestContent: ((NSFetchRequest<T>) -> Void)?,
